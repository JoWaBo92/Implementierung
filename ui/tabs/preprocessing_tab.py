...
class PreprocessingTab(QWidget):
    def __init__(self, project: Project, parent=None):
        ...
        self.transcript_result_box = self._build_transcript_result_box()
    ...
    def _build_controls_box(self) -> QGroupBox:
        ...
        self.combo_language_model.addItems(["de_core_news_sm","de_core_news_md","en_core_web_sm"])

        row1.addWidget(self.combo_language_model, 1)
        layout.addLayout(row1)

        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Optionen:"))
        ...
        row2.addWidget(self.chk_lowercase, 1)
        row2.addWidget(self.chk_keep_punct, 1)
    ...
    def _build_transcript_result_box(self) -> QGroupBox:
        box = QGroupBox("Preprocessing-Ergebnis")
        layout = QVBoxLayout(box)

        self.table_transcript_result = QTableWidget(0, 4)
        self.table_transcript_result.setHorizontalHeaderLabels(["#", "Original", "Normalisiert", "Tokens"])

        self._configure_table(self.table_transcript_result)
        self.table_transcript_result.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)

        layout.addWidget(self.table_transcript_result)
        return box
    ...
    def _build_extract_result_box(self) -> QGroupBox:
        ...
        self.table_extract_result = QTableWidget(0, 4)
        self.table_extract_result.setHorizontalHeaderLabels(["#", "Original", "Normalisiert", "Tokens"])

        self._configure_table(self.table_extract_result)
        self.table_extract_result.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)

        layout.addWidget(self.table_extract_result)
        return box
